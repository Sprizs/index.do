...
class checkcode {
	...
	public static $codeSet = '23456789ABCDEFGHJKLMNPQRTUVWXYZ';
	...
	public static function entry() {
		...
		for ($i = 0; $i < self::$length; $i++) {
			...
			// 写一个验证码字符
			imagettftext(self::$_image, self::$fontSize, mt_rand(0, 50), $codeNX, self::$fontSize * 1.5, self::$_color, $ttf, $code[$i]);
		}
		// 保存验证码
		...
		session_start();
		$_SESSION['checkcode'] = join("", $code); // 把验证码保存到session, 验证时注意是大写
...
//调用上面定义的验证码类 来生产验证码
CheckCode::$useNoise = true; //是否启用噪点
CheckCode::$useCurve = true; //是否启用干扰曲线
CheckCode::entry();
//核心代码结束